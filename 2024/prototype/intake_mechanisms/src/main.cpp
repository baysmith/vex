...
PivotRampPrototype makePivotRampPrototype() {
    ...
    // TODO: Must be determined experimentally.
    const double rotationsToTop = 0.5;
...
void updateClampState(ImobileGoalIntake& p) {
    // clamp means the air is released
    bool clamp = Controller.ButtonL1.pressing();
    // unclamp means the air is pumped in
    bool unclamp = Controller.ButtonL2.pressing();
    if (clamp) {
        p.clamp(true);
    } else if (unclamp) {
        p.clamp(false);
...
void teleop() {
    ...
    while (true) {
        ...
        // // move lift directly (rotationsToTop)
        // moveLiftRotationsToTopDebug(buttonUp, buttonDown, prototype);
        // // move lift directly (relative heights)
        // moveLiftGatherHeightsDebug(buttonUp, buttonDown, prototype);
        // // move lift by state machine (final)
        // updateLiftState(buttonUp, buttonDown, prototype, liftState);
